...
<template>
  ...
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-600/75">
    <div class="mx-4 max-h-96 w-full max-w-md overflow-y-auto rounded-lg bg-white shadow-xl">
      <!-- Modal Header -->
      ...
      <div class="px-6 py-4">
        ...
        <div v-else-if="!hasProfiles" class="py-6 text-center">
          ...
          <h4 class="mb-2 text-lg font-medium text-gray-900">No Job Profiles Found</h4>
          <p class="mb-4 text-gray-600">You need to create a job profile before using Turbo Apply.</p>
          <router-link to="/job-profiles" class="inline-flex items-center rounded-md border border-transparent bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-700" @click="closeModal"> Create Job Profile </router-link>
        </div>

        <!-- Profile Selection -->
        <div v-else-if="!selectedProfileId">
          <div class="mb-4">
            <label class="mb-2 block text-sm font-medium text-gray-700"> Select a job profile to apply with: </label>
            <select v-model="selectedProfileId" onchange="jobProfilesStore.validateProfile()" class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-500">
              <option value="">Choose a profile...</option>
              <option v-for="profile in jobProfilesStore.profiles" :key="profile.id" :value="profile.id">
                {{ profile.profile_name }}
                <span v-if="profile.first_name && profile.last_name"> ({{ profile.first_name }} {{ profile.last_name }}) </span>
              </option>
            </select>
        ...
        <div v-else-if="selectedProfileId">
          <small>
            {{ jobProfilesStore.selectedProfile?.profile_name }} profile selected.
            <span
      ...
      <!-- Modal Footer -->
      <div v-if="hasProfiles" class="flex justify-end space-x-3 border-t border-gray-200 px-6 py-4">
        <button class="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50" @click="closeModal">Cancel</button>
        <button :disabled="validating || loading" class="rounded-md border border-transparent bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50" @click="submit">
          Continue
        </button>
      </div>
    </div>
