<template>
  <div>
    <div class="mb-8">
      ...
      <p class="mt-2 text-base text-gray-600">Add up to 10 work experience entries. Include your most recent and relevant positions.</p>
    </div>
    ...
    <div class="space-y-8">
      ...
      <div v-if="experience.length >= 10" class="rounded-lg border border-amber-200 bg-amber-50 p-4">
        <div class="flex">
      ...
      <div v-if="experience.length > 0" class="space-y-6">
        <div v-for="(exp, index) in sortedExperience" :key="exp.id || `new-${index}`" class="rounded-xl border border-gray-200 bg-white p-8 shadow-sm transition-shadow hover:shadow-md">
          <!-- Experience Header -->
          <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <h4 class="text-lg font-semibold text-gray-900">
                ...
                <span v-if="exp.company_name" class="font-normal text-gray-600">at {{ exp.company_name }}</span>
              </h4>
            </div>
            <div class="flex items-center space-x-2">
              <button v-if="index > 0" class="p-1 text-gray-400 hover:text-gray-600" title="Move up" @click="moveExperience(index, 'up')">
                <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                </svg>
              </button>
              <button v-if="index < experience.length - 1" class="p-1 text-gray-400 hover:text-gray-600" title="Move down" @click="moveExperience(index, 'down')">
                <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              <button class="p-1 text-red-400 hover:text-red-600" title="Remove experience" @click="removeExperience(exp, index)">
                <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
            <!-- Job Title -->
            <div>
              <label :for="`job-title-${index}`" class="mb-2 block text-sm font-semibold text-gray-800"> Job Title * </label>
              <input
                :id="`job-title-${index}`"
            ...
            <!-- Company Name -->
            <div>
              <label :for="`company-name-${index}`" class="mb-2 block text-sm font-semibold text-gray-800"> Company Name * </label>
              <input
            ...
            <!-- Start Date -->
            <div>
              <label :for="`start-date-${index}`" class="mb-2 block text-sm font-semibold text-gray-800"> Start Date * </label>
              <input :id="`start-date-${index}`" v-model="exp.start_date" type="date" required class="block w-full rounded-lg border border-gray-300 px-4 py-3 text-base shadow-sm transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500" />
            </div>

            <!-- End Date / Current Job -->
            <div>
              <label :for="`end-date-${index}`" class="mb-2 block text-sm font-semibold text-gray-800"> End Date </label>
              <div class="space-y-3">
                <input
                  :id="`end-date-${index}`"
                ...
                <label class="inline-flex items-center">
                  <input v-model="exp.is_current_job" type="checkbox" class="size-4 rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200/50" @change="handleCurrentJobChange(exp)" />
          ...
          <!-- Description -->
          <div>
            <label :for="`description-${index}`" class="mb-3 block text-sm font-semibold text-gray-800"> Description </label>
            <QuillEditor v-model="exp.description" :placeholder="'Describe your responsibilities, achievements, and key contributions in this role...'" @update:model-value="handleDescriptionUpdate(exp, $event)" />
            <p class="mt-2 text-sm text-gray-500">Use bullet points to highlight key achievements and responsibilities. Content auto-saves as you type.</p>
          </div>
        </div>
      </div>

      <!-- Empty State -->
      <div v-else class="rounded-lg border-2 border-dashed border-gray-300 py-8 text-center">
