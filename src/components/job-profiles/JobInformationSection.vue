    ...
    <div class="space-y-8">
      ...
      <div>
        ...
        >
          ...
          <option value="fill_only_necessary">
            I want to fill out only what is necessary to apply
          </option>
          <option value="ai_autofill_with_prompt">
            Have AI auto fill my answer if I haven't seen it and prompt me
          </option>
          <option value="ai_autofill_no_interference">
            Have AI auto fill the answer without my interference
          </option>
        </select>
        <p class="mt-2 text-sm text-gray-500">
          Choose how you want to handle optional fields when using TurboApply.
          You can access all the fields you have already answered on your job
          profile.
        </p>
      </div>
      ...
      <div class="border-t border-gray-200 pt-8">
        ...
        <p class="mb-6 text-sm text-gray-600">
          Upload your resume and cover letter to include with applications
        </p>
            ...
            >
              <div class="flex items-center">
                ...
                <div class="ml-3 flex-1">
                  <p class="text-sm font-medium text-green-800">
                    {{
                      localProfile.cover_letter_filename ||
...
const localProfile = ref({
  ...
  optional_fields_handling:
    props.modelValue.optional_fields_handling || 'fill_unanswered_questions'
});
...
watch(
  ...
  (newValue) => {
    localProfile.value = {
      ...
      // Set default value for optional_fields_handling if not present
      optional_fields_handling:
        newValue.optional_fields_handling || 'fill_unanswered_questions'
    };
  },
  { deep: true }
);
...
const handleResumeUpload = async (event: Event) => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/job-profiles/${localProfile.value.id}/resume`,
      {
        method: 'POST',
        headers: {
...
const handleCoverLetterUpload = async (event: Event) => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/job-profiles/${localProfile.value.id}/cover-letter`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authStore.accessToken}`
...
const downloadResume = async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/job-profiles/${localProfile.value.id}/resume/download`,
      {
        headers: {
          Authorization: `Bearer ${authStore.accessToken}`
        }
...
const downloadCoverLetter = async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/job-profiles/${localProfile.value.id}/cover-letter/download`,
      {
        headers: {
          Authorization: `Bearer ${authStore.accessToken}`
        }
