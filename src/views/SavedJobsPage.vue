...
<template>
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="mb-6 flex items-center justify-between">
      ...
      <button v-if="bookmarkedJobs.length > 0" class="text-sm font-medium text-red-600 hover:text-red-700" @click="jobsStore.clearAllBookmarks()">Clear All Bookmarks</button>
    </div>

    <!-- Content -->
    <div v-if="loading || jobsStore.bookmarksLoading" class="flex items-center justify-center py-16">
    ...
    <div v-else-if="bookmarkedJobs.length === 0" class="rounded-lg bg-white p-8 text-center shadow-sm">
      ...
      <h3 class="mb-2 text-lg font-medium text-gray-900">No bookmarked jobs yet</h3>
      <p class="mb-4 text-gray-600">When you find a job that interests you, click the bookmark icon to save it for later.</p>
      <router-link to="/search" class="btn-primary"> Browse Jobs </router-link>
    </div>

    <div v-else>
      <p class="mb-4 text-gray-600">You have bookmarked {{ bookmarkedJobs.length }} job{{ bookmarkedJobs.length !== 1 ? 's' : '' }}</p>

      <div class="space-y-4">
        <div v-for="bookmark in bookmarkedJobs" :key="bookmark.bookmark_id" class="card mb-4 border border-gray-100 p-6 transition-all duration-300 hover:border-primary-100">
          <div class="flex flex-col justify-between sm:flex-row sm:items-start">
            <div class="flex-1">
              <div class="mb-2 flex items-start justify-between">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-red-500" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                  </svg>
                </button>
              </div>
              ...
              <div class="mb-4 mt-2">
                <div class="flex flex-wrap gap-2">
                  ...
                  <span v-if="bookmark.remote" class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800"> Remote </span>
                  <span v-if="bookmark.country" class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800">
                    {{ bookmark.country }}
                  </span>
                </div>
              </div>
              ...
              <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
                ...
                <a :href="bookmark.external_url" target="_blank" class="inline-flex items-center text-sm font-medium text-primary-600 transition-colors duration-200 hover:text-primary-700">
                  Apply Now
