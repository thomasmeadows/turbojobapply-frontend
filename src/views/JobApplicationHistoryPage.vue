  ...
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <div class="mb-4 sm:mb-0">
        <label for="ats-filter" class="mb-1 block text-sm font-medium text-gray-700"> Filter by ATS Source </label>
        <select id="ats-filter" v-model="selectedAtsSource" class="rounded-md border border-gray-300 px-3 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-500" @change="onAtsSourceChange">
    ...
    <div v-if="error" class="mb-6 rounded-md border border-red-200 bg-red-50 p-4">
      <div class="flex">
        <svg class="mr-2 size-5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Empty State -->
    <div v-else-if="applications.length === 0" class="py-16 text-center">
      <svg class="mx-auto mb-4 size-16 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <router-link to="/search" class="inline-flex items-center rounded-md border border-transparent bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-700"> Browse Jobs </router-link>
    </div>

    <!-- Applications List -->
    <div v-else class="space-y-4">
      <div v-for="application in applications" :key="application.id" class="rounded-lg border border-gray-200 bg-white transition-colors duration-200 hover:border-gray-300">
        <div class="p-6">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="mb-2 flex items-center">
                ...
                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium" :class="statusBadgeClasses[application.application_status]">
              ...
              <div v-if="application.job_data.description" class="mt-3 text-sm text-gray-600">
                <div class="line-clamp-2" v-html="application.job_data.description.substring(0, 200) + (application.job_data.description.length > 200 ? '...' : '')" />
              </div>
            </div>

            <div class="ml-4 flex items-center space-x-2">
              <button v-if="application.job_data.external_url" class="text-sm font-medium text-primary-600 hover:text-primary-700" @click="viewJobDetails(application)">View Job</button>
              <button class="text-sm font-medium text-gray-600 hover:text-gray-700" @click="viewApplicationDetails(application.id)">Details</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="totalPages > 1" class="mt-8 flex items-center justify-between">
      <div class="text-sm text-gray-700">Showing {{ (currentPage - 1) * limit + 1 }} to {{ Math.min(currentPage * limit, totalCount) }} of {{ totalCount }} applications</div>

      <div class="flex items-center space-x-2">
        <button :disabled="!hasPreviousPage" class="rounded-md border border-gray-300 px-3 py-1 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" @click="goToPage(currentPage - 1)">Previous</button>
        ...
        <div class="flex items-center space-x-1">
          <template v-for="page in Math.min(5, totalPages)" :key="page">
            <button
              ...
              :class="page === currentPage ? 'border-primary-600 bg-primary-600 text-white' : 'border-gray-300 text-gray-700 hover:bg-gray-50'"
              @click="goToPage(page)"
            >
              {{ page }}
            </button>
          ...
          <button v-if="totalPages > 5 && currentPage < totalPages - 2" class="rounded-md border border-gray-300 px-3 py-1 text-sm font-medium text-gray-700 hover:bg-gray-50" @click="goToPage(totalPages)">
            {{ totalPages }}
          </button>
        </div>

        <button :disabled="!hasNextPage" class="rounded-md border border-gray-300 px-3 py-1 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" @click="goToPage(currentPage + 1)">Next</button>
      </div>
    </div>
  </div>
</template>
