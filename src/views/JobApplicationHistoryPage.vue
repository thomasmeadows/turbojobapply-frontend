...
const getCompanyName = (application: JobApplication) => {
  return (
    application.job_data.company ||
    application.job_data.client_name ||
    'Unknown Company'
  );
};
...
const fetchApplicationHistory = async (page: number = 1) => {
  try {
    ...
    const response = await axios.get(
      `${API_URL}/api/job-applications/history?${params}`,
      {
        headers: {
    ...
    error.value =
      err.response?.data?.message || 'Failed to load application history';
  } finally {
    loading.value = false;
  }
};
    ...
    >
      <div class="mb-4 sm:mb-0">
        ...
        <select
          ...
          class="rounded-md border border-gray-300 px-3 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-500"
          @change="onAtsSourceChange"
        >
          <option
            v-for="option in atsSourceOptions"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
        ...
        <div class="p-6">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div
                v-if="application.job_data.description"
                class="mt-3 text-sm text-gray-600"
              >
                <div
                  class="line-clamp-2"
                  v-html="
                    application.job_data.description.substring(0, 200) +
                    (application.job_data.description.length > 200 ? '...' : '')
                  "
                />
              </div>
            </div>
